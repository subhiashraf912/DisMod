...
const discordVersion = parseInt(discord_js_1.version);
class AutoModeratorManager extends tiny_typed_emitter_1.TypedEmitter {
  constructor(client, options) {
    ...
    const eventName = discordVersion >= 13 ? "messageCreate" : "message";
    client.on(eventName, (message) =>
      __awaiter(this, void 0, void 0, function* () {
        var _a, _b, _c;
        if (
          ((options === null || options === void 0 ? void 0 : options.adminCheck) && ((_a = message.member) === null || _a === void 0 ? void 0 : _a.hasPermission("ADMINISTRATOR"))) ||
        ...
        if (!emojiCheck) emojiCheck = [];
        const saidBannedWords = checkers_1.CheckBannedWords(options, message.content);
        //
        if (saidBannedWords[0]) yield this.emit("badWordUsage", message, saidBannedWords);
        //
        if (checkers_1.checkRepeatedText(message.content)[0] && emojiCheck.length <= 4) this.emit("repeatedText", message, checkers_1.checkRepeatedText(message.content).length);
        //
        const capsAmount = checkers_1.CheckCaps(message.content);
        this.emit("capsCheck", message, capsAmount);
        //
        if (emojiCheck && emojiCheck.length >= 4) this.emit("emojisSpam", message, emojiCheck.length);
        //
        if (CheckExternalLinks_1.default(message.content)[0] && !message.content.includes("discord.gg")) this.emit("externalLink", message, CheckExternalLinks_1.default(message.content));
        //
        if (CheckExternalLinks_1.default(message.content)[0] && message.content.includes("discord.gg" || "discordapp.com/invite/"))
        ...
        //
        if (message.mentions.users.size >= 4) this.emit("mentionsSpam", message, message.mentions.users.size);
        //
        const spoilersAmount = (message.content.split(" ").join("").split("||").length - 1) / 2;
        if (spoilersAmount >= 3) this.emit("spoilersSpam", message, spoilersAmount);
      })
    );
  }
}
